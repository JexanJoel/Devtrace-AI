...
const DashboardPage = () => {
  ...
  const analyzedCount = sessions.filter(s => s.ai_analysis != null).length;
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-2xl p-6 flex items-center justify-between flex-wrap gap-4"
          ...
          <div>
            ...
            <p className="text-indigo-200 text-sm mt-1">
              {loading ? 'Loading...' : stats?.totalSessions === 0
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6">
            ...
            {loading ? [...Array(3)].map((_, i) => (
              <div key={i} className="flex gap-3 animate-pulse mb-3">
                ...
                <div className="flex-1 space-y-1.5"><div className="h-4 bg-gray-100 dark:bg-gray-800 rounded w-3/4" /><div className="h-3 bg-gray-100 dark:bg-gray-800 rounded w-1/2" /></div>
              </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6">
            ...
            {loading ? [...Array(3)].map((_, i) => (
              <div key={i} className="animate-pulse flex gap-3 p-3 mb-2">
                ...
                <div className="flex-1 space-y-1.5"><div className="h-4 bg-gray-100 dark:bg-gray-800 rounded w-1/2" /><div className="h-3 bg-gray-100 dark:bg-gray-800 rounded w-1/3" /></div>
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button onClick={() => navigate('/ai-insights')}
            ...
            <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-900 rounded-2xl flex items-center justify-center flex-shrink-0">
              <Sparkles size={22} className="text-indigo-600" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-bold text-gray-900 dark:text-white">AI Insights</p>
              <p className="text-xs text-gray-500 mt-0.5">Category breakdown, confidence scores, flagged files</p>
            </div>
            <ArrowRight size={16} className="text-gray-300 group-hover:text-indigo-500 transition flex-shrink-0" />
          </button>

          <button onClick={() => navigate('/sync-status')}
            className="flex items-center gap-4 bg-gradient-to-r from-violet-50 to-blue-50 dark:from-violet-950 dark:to-blue-950 border border-violet-100 dark:border-violet-900 rounded-2xl p-5 hover:shadow-sm hover:border-violet-300 transition text-left group">
            <div className="w-12 h-12 bg-violet-100 dark:bg-violet-900 rounded-2xl flex items-center justify-center flex-shrink-0">
              <Database size={22} className="text-violet-600" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-bold text-gray-900 dark:text-white">Sync Status</p>
              <p className="text-xs text-gray-500 mt-0.5">PowerSync architecture, live read counts, sync queue</p>
            </div>
            <ArrowRight size={16} className="text-gray-300 group-hover:text-violet-500 transition flex-shrink-0" />
          </button>
        </div>
