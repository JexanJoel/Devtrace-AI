...
const RegisterPage = () => {
  ...
  const handleRegister = async () => {
    ...
    if (data.session) {
      // Also save name to profiles table
      await supabase.from('profiles').update({ name }).eq('id', data.user!.id);
      toast.success('Account created! Welcome to DevTrace 🎉');
      navigate('/dashboard');
    } else {
      // Confirmation is still ON in Supabase dashboard — tell user to turn it off
      toast.error('Email confirmation is enabled in Supabase. Go to Auth → Settings and disable "Confirm email".');
    }
        ...
        <div className="relative z-10 flex-1 flex items-center justify-center px-10">
          <div className="w-full max-w-xs space-y-6 text-left">
            <div className="space-y-3">
              <h2 className="text-3xl font-bold text-white leading-tight">Debug smarter, ship faster.</h2>
              <p className="text-indigo-200 text-sm leading-relaxed">AI-powered debugging for developers. Track sessions, get fixes, ship faster.</p>
            </div>
            <div className="flex flex-col gap-2">
              {features.map((f, i) => (
                <div key={i} className="flex items-center gap-3 bg-white/10 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm">
                  <span className="text-indigo-200 flex-shrink-0">{f.icon}</span>{f.text}
                </div>
              ))}
            </div>
