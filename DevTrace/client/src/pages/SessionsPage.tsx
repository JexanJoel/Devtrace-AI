...
const SessionsPage = () => {
  ...
  const filtered = sessions.filter((s) => {
    const matchesSearch = s.title.toLowerCase().includes(search.toLowerCase()) || s.error_message?.toLowerCase().includes(search.toLowerCase());
  ...
  const handleExportAll = () => {
    if (!sessions.length) { toast.error('No sessions to export'); return; }
  ...
  return (
    <DashboardLayout title="Debug Sessions">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <button onClick={handleExportAll}
              className="flex items-center gap-2 border border-gray-200 dark:border-gray-700 hover:border-gray-300 text-gray-600 dark:text-gray-400 font-medium px-4 py-2.5 rounded-xl text-sm transition">
        ...
        <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl w-fit flex-wrap">
          {FILTERS.map((f) => (
            <button key={f.value} onClick={() => setFilter(f.value)}
              ...
              {f.label}
              <span className={`text-xs px-1.5 py-0.5 rounded-md ${filter === f.value ? 'bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-300' : 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>
                {counts[f.value]}
              </span>
            </button>
          ))}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h3 className="font-bold text-gray-900 dark:text-white mb-1">{search || filter !== 'all' ? 'No sessions found' : 'No sessions yet'}</h3>
            <p className="text-gray-400 text-sm mb-6 max-w-xs">
              {search || filter !== 'all' ? 'Try adjusting your search or filter' : 'Log your first debug session to start tracking errors'}
            </p>
            {!search && filter === 'all' && (
              <button onClick={() => setShowModal(true)}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-50 dark:border-gray-800">
              <p className="text-xs text-gray-400 font-medium">{filtered.length} session{filtered.length !== 1 ? 's' : ''}</p>
            </div>
      ...
      {showModal && <CreateSessionModal onClose={() => setShowModal(false)} onCreate={createSession} />}
    </DashboardLayout>
  );
};
